...
class LearningAgent(Agent):
	...
	if testing == True:
		...
		#self.epsilon *= 1.0/(self.x**3+self.x*2)
		#self.epsilon -= math.fabs(math.cos(self.alpha*self.x))
		#self.epsilon -= math.exp(-0.02*self.trail_number)
                #self.trial_num = self.trail_number + 1
	...
	del inputs['right'] 
	inp = tuple([j for i,j in inputs.items()])       
        state = (inp,waypoint)
	...
	if self.learning is True:
		req_action_value = self.get_maxQ(state)
		action_choices = self.Q[state]
		...
		if flip <= self.epsilon:
			...
			lis = []
			for i,j in action_choices.items():
				if j== req_action_value:  #Tie-Breaker Condition
					lis.append(i)
			action = random.choice(lis)
	else:	
        	action = random.choice(self.valid_actions)
